    ...
    .then((data) => {
      ...
      if (data.url) {
        ...
        document.getElementById(
          "uploadedImage"
        ).innerHTML = `<img src="${data.url}" alt="Uploaded Image" />`;
        ...
        document.createElement("button").innerHTML = "download";
        document.getElementById("processingActions").appendChild(download);
      }
    });
}
  ...
  fetch("/process", {
    ...
    .then((data) => {
      if (data.processed_url) {
        document.getElementById(
          "processedImage"
        ).innerHTML = `<h3>Processed Image</h3><img src="${data.processed_url}" alt="Processed Image" />`;
        if (!document.getElementById("downloadButton")) {
          const downloader = document.createElement("button");
          downloader.id = "downloadButton";
          downloader.innerText = "Download";
          downloader.onclick = () => downloadImage();
          processingActions.innerHTML = "";
          processingActions.appendChild(downloader);
        }
      }
    });
}
